    ...
    def step(self, update_stats=True, update_params=True, grads=None):
        ...
        for group in self.param_groups:
            ...
            if group['layer_type'] == 'Conv2d' and self.sua:
                _norm, nat_bgrad, nat_wgrad = self._precond_sua_ra(weight, bias, group, state, wgrad, bgrad)
            elif group['layer_type'] == 'BatchNorm2d':
                _norm, nat_bgrad, nat_wgrad = self._precond_diag_ra(weight, bias, group, state, wgrad, bgrad)
            else:
                _norm, nat_bgrad, nat_wgrad = self._precond_ra(weight, bias, group, state, wgrad, bgrad)
