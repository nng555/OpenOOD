...
class EKFAC(Optimizer):
    ...
    def __init__(self, net, eps, sua=False, layer_eps=True, eps_type='mean'):
        ...
        for name, mod in net.named_modules():
            ...
            if mod_class in ['Linear', 'Conv2d', 'BatchNorm2d', 'GroupNorm']:
                params = [mod.weight]
    ...
    def step(self, update_stats=True, update_params=True, grads=None, return_grads=False):
        ...
        for group in self.param_groups:
            ...
            if grads is not None:
                wname, wgrad = grads[weight]
                if bias is not None:
                    bname, bgrad = grads[bias]

            if group['layer_type'] == 'Conv2d' and self.sua:
                out = self._precond_sua_ra(weight, bias, group, state, wgrad, bgrad, return_grads=return_grads)
            elif group['layer_type'] == 'BatchNorm2d' or group['layer_type'] == 'GroupNorm':
                out = self._precond_diag_ra(weight, bias, group, state, wgrad, bgrad, return_grads=return_grads)
            else:
                out = self._precond_ra(weight, bias, group, state, wgrad, bgrad, return_grads=return_grads)

            if return_grads:
                _norm, nat_bgrad, nat_wgrad = out
    ...
    def _precond_ra(self, weight, bias, group, state, g=None, gb=None, return_grads=False):
        ...
        norm = torch.sum(g_kfe**2 / (m2 + eps), list(range(1, g_kfe.ndim))) / self.numel
    ...
    def _precond_sua_ra(self, weight, bias, group, state, g=None, gb=None, return_grads=False):
    ...
    def _precond_diag_ra(self, weight, bias, group, state, g=None, gb=None, return_grads=False):
        res = []
        if g is None:
            g = weight.grad.data.unsqueeze(0)
        fw = state['exact_fw']
        ...
        norm = torch.sum(g**2 / (fw + eps), list(range(1, g.ndim))) / self.numel
        g_nat = g / (fw + eps)
        ...
        if bias is not None:
            ...
            gb_nat = gb / (fb + eps)
            norm += torch.sum(gb**2 / (fb + eps), list(range(1, gb.ndim))) / self.numel
    ...
    def update_cache(self, op, **kwargs):
        for group in self.param_groups:
            ...
            is_diag = group['layer_type'] == 'BatchNorm2d' or group['layer_type'] == 'GroupNorm'
